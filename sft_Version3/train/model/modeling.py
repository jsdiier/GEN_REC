...
输入约定（与 step3/step4 的序列构造对齐；每个交互 = [1 时段 token +] 1 行为 token +
tokens_per_item 个 SID token，时段位仅在 num_periods>0 时存在）：
  input_ids    : (B, L) 统一词表 id
  behavior_ids : (B, L) 每个 token 所属 item 的行为 id（0=clk, 1=pay, ...）；
                 非 item token（BOS/PAD 等 special）= -1（时段 token 沿用所属交互的行为 id，
                 跨层行为交互中与行为 token 同等参与）
  token_types  : (B, L) token 在 item 内的位置：0=行为 token, 1..l=第 j 层 SID token,
                 l+1=时段 token（走 MoE 的时段专属专家）；
...
class GAMERConfig:
    ...
    num_periods: int = 0               # 时段种类（bf/lc/dn=3；0=序列不含时段位）
...
class CrossLevelBehaviorInteraction(nn.Module):
    ...
    def forward(self, x, behavior_ids, levels, pad_mask):
        ...
        mask = (causal_strict[None] & level_ok
                ...
                & pad_mask[:, None, :])                           # (B,L,L)
        # 用 sdpa 省显存（不物化 (B,H,L,L) 的 attention 矩阵）。
        # sdpa 对整行全 False 的 mask 会输出 NaN：先让这类 query（如序列开头的
        # 低层行为、padding）暂时 attend 自身，算完再把它们的输出置 0。
        no_key = ~mask.any(dim=-1)                                # (B,L)
        eye = torch.eye(L, dtype=torch.bool, device=x.device)
        safe_mask = mask | (no_key[:, :, None] & eye[None])
        out = F.scaled_dot_product_attention(q, k, v, attn_mask=safe_mask[:, None])
        out = out.masked_fill(no_key[:, None, :, None], 0.0)
        out = out.transpose(1, 2).reshape(B, L, H * hd)
...
class PositionBehaviorMoE(nn.Module):
    """按 token 在 item 内的位置路由固定专家：
       ...
       expert_period (type=l+1, 仅 num_periods>0): 时段 token，输入 D（上下文位，
       与行为位同构但参数独立）。"""
    ...
    def __init__(self, cfg: GAMERConfig):
        ...
        self.period_type = l + 1
        self.expert_period = SwiGLU(d, inner, d) if cfg.num_periods > 0 else None

    def forward(self, x, behavior_ids, token_types):
        """x: (B,L,D); token_types: (B,L) 0=行为, 1..l=SID第j层, l+1=时段, -1=非item。"""
        ...
        if self.expert_period is not None:
            mp = token_types == self.period_type
            if mp.any():
                out[mp] = self.expert_period(x[mp]).to(out.dtype)
        return out
