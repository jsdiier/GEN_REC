...
# -*- coding: utf-8 -*-
"""
submit_train: 把 GAMER 训练任务提交到 Luban 平台，并轮询到终态。

由统一入口 train/run_train.sh 在 [train] launch_mode=platform 时调用；
平台侧执行 [platform] script_path（train_platform.sh，NFS 路径），
scriptParam 传平台容器内的 common.conf 路径——两侧共用同一份配置文件
（同一块 NFS 盘的两个挂载名），本地改完 conf 提交即生效。

用法:
    python submit_train.py [common.conf] [--dry-run]
...
def load_platform_config(conf_path: str) -> dict:
    ...
    pc = {
        ...
        "script_path": f"{project_dir}/train/train_platform.sh",
        "poll_interval": cp.getint("platform", "poll_interval", fallback=60),
        "job_name_prefix": g("job_name_prefix", "gamer_train"),
        "level": g("level", "PRO"),
...
def build_payload(pc: dict, job_name: str) -> dict:
    # scriptParam = 平台容器内的 common.conf 路径（train_platform.sh 的 $1）
