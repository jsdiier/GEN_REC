...
供 train/dataset 读取，落盘只留 (action, geo_sid)，留空则仅打印诊断）：
  ...
  7. val/test 的形态（同构）：每用户各一条样本，label_tokens = label session 的
     ...
     只算 label 区 loss；test 下游用 input 做前缀推理，与 positives_by_action
     （按行为去重的正样本集合）比对算 HR/Recall/NDCG。
...
def _make_eval_sample(uid, split, input_tokens, label_date, label_tokens) -> dict:
    """val/test 同构样本（每用户各一条）。label_tokens 为 label session 原始有序
       交互（val 走 teacher-forcing 算 loss）；positives_by_action 为按行为去重的
       正样本集合（test 算 HR/Recall/NDCG）。"""
    return {
        "uid": uid,
        "split": split,
        "input": input_tokens,
