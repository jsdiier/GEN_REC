...
流程（不落盘、全程流式）：
  ...
  6. train 的 behavior-drop 增强（对齐论文 3.3）：x = behavior_drop_x，
       ...
       每用户共输出 x+1 条 train 序列（原始 + x 个变体）；val 不增强。
  7. val 的 label 分割：label 区按「目标行为 b」取正样本集合 𝒯u（去重），
     每条样本 = (input, target_action, label_geo_sids 集合)，用于 checkpoint 选择 /
     早停时算 HR/Recall/NDCG（或直接算 val loss）。
...
def _make_eval_samples(uid, split, input_tokens, label_date, label_tokens):
    """val：label 区内每个目标行为 b 出一条样本，label = 该 b 的正样本集合。"""
    samples = []
...
def build_all_samples(uid: str, sessions: list, behavior_drop_x: int) -> list:
    """session 粒度留一法两分（m>=2 的用户才保留；test 由独立窗口流程另建）：
       val   : input=S1..S(m-1), label=Sm         （评测样本，label 为正样本集合）
       train : S1..S(m-1) 全序列 + x 个 drop 变体 （NTP 序列，无 label 区）
    ...
    out = []
    # val：train 窗口内每用户最后一个 session 当 label 区
    val_input = [t for _, toks in sessions[:m - 1] for t in toks]
    out += _make_eval_samples(uid, "val", val_input, sessions[m - 1][0], sessions[m - 1][1])
...
def main():
    ...
    total_pos = Counter()             # val 正样本(去重后)累计，估平均集合大小
    ...
    print("val（评测样本，每条 = (input, target_action, 正样本集合)）:")
    cnt = split_samples.get("val", 0)
    avg = (total_pos.get("val", 0) / cnt) if cnt else 0.0
    print(f"  val  : {cnt} 条  (平均正样本集合大小 {avg:.2f})")
    print("=====================================")
