...
[inference] gpu_num>1 时是数据并行模式：提交前先调用 hdfs_utils 做一次性的
baseline 合并（把历史结果不管是老的单文件还是之前不同 gpu_num 留下的 part
文件，统一合并成一份 _baseline 快照，避免残留旧 part 文件造成同一 cache_key
重复），然后并行提交 gpu_num 个独立单 GPU 任务（每个任务的 scriptParam 在
...
def _submit_sharded(pc: dict, conf_path: str, gpu_num: int, dry_run: bool) -> None:
    ...
    print(f"[INFO] 项目: {pc['project_name']}  平台目录: {pc['platform_project_dir']}")
    print(f"[INFO] 数据并行 gpu_num={gpu_num}  target_dt={infer_end}")
